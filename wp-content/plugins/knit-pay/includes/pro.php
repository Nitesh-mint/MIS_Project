...
class KnitPayPro {
	...
	public static function knit_pay_pro_payment_status_update( $payment ) {
		...
		$invalid_gateways = [ 'instamojo', 'manual', 'go-url', 'razorpay', 'sslcommerz', 'upi-qr', 'stripe-connect' ];
		...
		if ( in_array( $gateway, $invalid_gateways, true ) && in_array( $source, $invalid_sources, true ) ) {
			return;
		}

		if ( 'test' === $payment->get_mode() || 'test' === $gateway ) {
			return;
		}
