...
class Integration extends AbstractGatewayIntegration {
	...
	public function __construct( $args = [] ) {
		...
		// Show notice if API keys getting used instead of OAuth.
		add_action( 'admin_notices', [ $this, 'razoray_deprecated_keys_notice' ] );
	}
	 ...
	 * @return void
	 */
	public function razoray_deprecated_keys_notice() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}

		// Don't show message if Woocommerce is installed to minimize impact.
		...
		/*
		 $random_day  = get_transient( 'knit_pay_razoray_deprecated_keys_rand_day' );
		$current_day = gmdate( 'N' );
		if ( empty( $random_day ) ) {
			$random_day = strval( wp_rand( 1, 7 ) );
			set_transient( 'knit_pay_razoray_deprecated_keys_rand_day', $random_day, MONTH_IN_SECONDS );
		}
		...
		if ( empty( $config_ids ) ) {
			...
			$query = new WP_Query(
				[
					...
					'meta_query' => [
						...
						[
							'key'     => '_pronamic_gateway_razorpay_key_secret',
							'compare' => 'EXISTS',
						],
					],
				]
			...
			set_transient( 'knit_pay_razorpay_with_deprecated_keys', $config_ids, DAY_IN_SECONDS );
		}

		if ( ! empty( $config_ids ) ) {
			include __DIR__ . '/views/notice-deprecated_keys.php';
		}
	}
	...
	public function get_settings_fields() {
		...
		// Razorpay on Hold.
		if ( ! defined( 'KNIT_PAY_RAZORPAY_CONNECT' ) && ( ! isset( $this->config ) || empty( $this->config->key_id ) ) ) {
			$fields[] = [
				'section'  => 'general',
				'type'     => 'custom',
				'title'    => 'Please Note',
				'callback' => function () {
					echo '<h1><strong>RBI asks Razorpay to pause onboarding merchants.</strong></h1>' .
					'<br><a class="button button-primary button-large" target="_blank" href="https://indianexpress.com/article/business/banking-and-finance/rbi-asks-razorpay-to-pause-onboarding-merchants-8329064/"
			...
			$fields[] = [
				...
				'type'     => 'custom',
				'callback' => function () {
					echo '<h1>We recommend you use <strong>Instamojo</strong> for the time being. Thanks for your patience.</h1>' .
		...
		// If access token available, switch to connect mode.
		if ( ! empty( $this->config->access_token ) ) {
			$GLOBALS['KNIT_PAY_RAZORPAY_API']     = false;
			$GLOBALS['KNIT_PAY_RAZORPAY_CONNECT'] = true;
		}
		...
		if ( $GLOBALS['KNIT_PAY_RAZORPAY_API'] && ! defined( 'KNIT_PAY_RAZORPAY_SUBSCRIPTION' ) ) {
		...
		if ( $GLOBALS['KNIT_PAY_RAZORPAY_API'] ) {
			// Key ID.
			$fields[] = [
				'section'  => 'general',
	...
	public function save_post( $config_id ) {
		...
		// Delete and recheck razorpay configurations with deprecated keys.
		delete_transient( 'knit_pay_razorpay_with_deprecated_keys' );

		$config = $this->get_config( $config_id );

		if ( empty( $config->key_secret ) && ! defined( 'KNIT_PAY_RAZORPAY_CONNECT' ) ) {
			return;
		}
